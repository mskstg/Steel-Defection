...
net = net.to(device)
PATH = './weight/weight_gpu_vgg19_resize_multi_weight.pth'
net.load_state_dict(torch.load(PATH))

trans = transforms.Compose([transforms.Resize(224),
                            transforms.ToTensor(),
                            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
test_set = torchvision.datasets.ImageFolder(root='./multi/test',
                                            transform=trans)
...
def main():
    ...
    total = 0
    cls_total = [0, 0, 0, 0, 0]
    cls_correct = [0, 0, 0, 0, 0]
    cls_predict = [0, 0, 0, 0, 0]
    with torch.no_grad():
